function [y,atomMatrix,activationMat,mX,pX] = stft_NNMF(x,N,ploton,K,iterations)
% Analysis and Synthesis of a sound using the short-time fourier transform
% x:            input sound,
% N:            FFT size
% y:            output sound
% window:       window function
% ploton:       plot or not the NNMF process
% K:            number of atoms
% iterations:   iterations to find atoms and activation matrices
...
sumW          = sum(atomMatrix);
atomMatrix    = atomMatrix*diag(1./sumW);
activationMat = diag(sumW) * activationMat;
